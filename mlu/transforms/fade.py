...
class Fade(Transform):
	...
	def _check_attributes(self):
		if not isinstance(self.factor, float) and not (isinstance(self.factor, tuple) and len(self.factor) == 2):
			raise ValueError(f'Invalid factor "{self.fill_value}", must be a float or a tuple of 2 floats.')
