...
from mlu.transforms.waveform.crop import crop, crop_align_random
from mlu.transforms.waveform.pad import pad, pad_align_random
...
class PadCropRandom(WaveformTransform):
	def __init__(
		self,
		target_length: int,
		fill_value: float = 0.0,
	...
	def process(self, waveform: Tensor) -> Tensor:
		waveform = pad_crop_random(waveform, self.target_length, self.fill_value, self.dim)
		return waveform
...
def pad_crop_random(waveform: Tensor, target_length: int, fill_value: float, dim: int) -> Tensor:
	waveform = pad_align_random(waveform, target_length, fill_value, dim)
	waveform = crop_align_random(waveform, target_length, dim)
	return waveform
