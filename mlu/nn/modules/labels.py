...
class MultiHot(Module):
	def __init__(self, nb_classes: int, smooth: Optional[float] = 0.0, dtype: torch.dtype = torch.float):
		super().__init__()
		self.nb_classes = nb_classes
		self.smooth = smooth if smooth is not None else 0.0
		self.dtype = dtype
	...
	def forward(self, x: Union[np.ndarray, Tensor]) -> Union[np.ndarray, Tensor]:
		return nums_to_smooth_multihot(x, self.nb_classes, self.smooth, self.dtype)
