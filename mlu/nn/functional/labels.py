...
# --- ONE-HOT ---
def nums_to_onehot(nums: Union[np.ndarray, Tensor], n_classes: int) -> Union[np.ndarray, Tensor]:
	"""
		Convert numbers (or indexes) of classes to one-hot version.

		:param nums: Label of indexes of classes.
		:param n_classes: The maximum number of distinct classes.
		:returns: Label with one-hot vectors
	"""
	if isinstance(nums, Tensor):
		onehot_vectors = one_hot(nums, n_classes)
	elif isinstance(nums, np.ndarray):
		onehot_vectors = nums_to_onehot(torch.from_numpy(nums), n_classes).cpu().numpy()
	else:
		onehot_vectors = nums_to_onehot(torch.as_tensor(nums), n_classes)
	return onehot_vectors


def nums_to_smooth_onehot(nums: Union[np.ndarray, Tensor], n_classes: int, smooth: float) -> Union[np.ndarray, Tensor]:
	"""
		Convert numbers (or indexes) of classes to smooth one-hot version.

		:param nums: Label of indexes of classes.
		:param n_classes: The maximum number of distinct classes.
		:param smooth: The label smoothing coefficient in [0, 1/n_classes].
		:returns: Label with smooth one-hot vectors
	"""
	onehot_vectors = nums_to_onehot(nums, n_classes)
	return onehot_to_smooth_onehot(onehot_vectors, n_classes, smooth)
...
def onehot_to_smooth_onehot(
	onehot_vectors: Union[np.ndarray, Tensor], n_classes: int, smooth: float
) -> Union[np.ndarray, Tensor]:
	""" Smooth one-hot labels with a smoothing coefficient. """
	classes_smoothed = (1.0 - smooth) * onehot_vectors + smooth / n_classes
...
def binarize_pred_to_onehot(pred: Tensor, dim: int = 1) -> Tensor:
	...
	n_classes = pred.shape[dim]
	onehot_vectors = one_hot(indexes, n_classes)
	return onehot_vectors
...
def nums_to_matrix(nums_lst: List[List[int]], fill_value: float = -1.0) -> Tensor:
	...
	for nums in nums_lst:
		n_classes = max(n_classes, len(nums))
	result = torch.full(size=(bsize, n_classes), fill_value=fill_value)
...
def nums_to_smooth_multihot(
	...
	multihot_vectors = nums_to_multihot(nums_lst, n_classes, dtype)
...
def multihot_to_smooth_multihot(
	...
	"""
	n_classes = multihot_vectors.shape[-1]
	result = (1.0 - smooth) * multihot_vectors + smooth / n_classes
	return result
