...
def balanced_split(
	...
	indexes_s, remaining_sample = _fill_subset(remaining_sample, s_expected_occur, n_classes, verbose)

	# For the unsupervised subset, if automatic set, then it is the remaining samples
	if ratio_u + ratio_s == 1.0:
		indexes_u = [idx for _target, idx in remaining_sample]
	else:
		indexes_u, _ = _fill_subset(remaining_sample, u_expected_occur, n_classes, verbose)

	return indexes_s, indexes_u


def get_multihots(dataset: SizedDataset, n_classes: int, target_type: str = 'indexes') -> Tensor:
	"""
		:param dataset: TODO
		:param n_classes: TODO
		:param target_type: 'indexes' or 'multihot'
	...
	if not hasattr(dataset, 'get_target'):
		raise RuntimeError(f'Dataset "{type(dataset)}" must have a method "get_target(index)" for getting multihots targets.')

	n_samples = len(dataset)

	targets = [
		torch.as_tensor(dataset.get_target(i))
...
def get_indexes_per_class(
	...
	"""
	if not hasattr(dataset, 'get_target'):
		raise RuntimeError(f'Dataset "{type(dataset)}" must have a method "get_target(index)" for getting indexes per class.')
...
def get_multihots_from_indexes_per_class(indexes_per_class: Sequence[Sequence[int]]) -> Tensor:
	max_idx = -1
...
def _fill_subset(
		...
		while idx < len(remaining_samples) and subset_occur[class_idx] < expected[class_idx]:
			if remaining_samples[idx][0][class_idx] == 1:
				...
				if progress is not None:
					progress.update(target.sum().item())
	...
	return torch.as_tensor(subset), remaining_samples
