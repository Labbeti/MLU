...
class FSD50KDataset(Dataset):
	...
	def _prepare_dataset(self):
		...
		for i, (fname, info) in enumerate(files_info.items()):
			...
			if not osp.isfile(fpath):
				...
				download_url(url, root_dir, fname, hash_expected, hash_type, progress_bar=self._verbose > 0)
			else:
		...
		if not osp.isfile(fpath_unsplit):
			subprocess.check_call(['zip', '-s', '0', fpath_split, '--out', fpath_unsplit])
	...
	def _is_prepared(self) -> bool:
		...
		return all([
			...
			len(os.listdir(osp.join(root_dir, 'FSD50K.dev_audio'))) == SUBSET_INFO['dev']['n_samples'],
			len(os.listdir(osp.join(root_dir, 'FSD50K.eval_audio'))) == SUBSET_INFO['eval']['n_samples'],
		])

	def _load_data(self):
		if not self._is_prepared():
			raise RuntimeError(f'Dataset is not downloaded in directory "{self.get_root_dir()}" and download=False.')
		...
		fpath_ground_truth = osp.join(root_dir, SUBSET_INFO[self._subset]['ground_truth'])
