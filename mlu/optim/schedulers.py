...
class CosineLRScheduler(LambdaLR):
	def __init__(self, optimizer: Optimizer, num_steps: int, coefficient: float = 7.0 / 16.0):
		"""
			...
			:param num_steps: The number of step() method call. Can be the number of epochs or iteration.
...
class SoftCosineLRScheduler(LambdaLR):
	def __init__(self, optimizer: Optimizer, num_steps: int):
		"""
			Scheduler that decreases the learning rate from lr0 to almost 0 by using the following rule :
